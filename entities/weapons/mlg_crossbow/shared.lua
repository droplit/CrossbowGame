...
print("Crossbow SWEP loaded!")


SWEP.Base				= "weapon_base"

SWEP.PrintName			= "MLG Crossbow"	
SWEP.ClassName			= "weapon_mlgcrossbow"		
SWEP.Author				= "Droplit"
SWEP.Category			= "MLG Weapons"
SWEP.Instructions		= "Hold left click to charge up your shot and fire different arrows.  Right click to throw a grapple arrow, space to reel in, R to release all of your grapples."
SWEP.Slot				= 2
SWEP.SlotPos			= 3
...
function SWEP:DrawHUD()
	...
	surface.DrawRect(ScrW()/2 - 105, ScrH()-37.5, ScrH()/2, 30)
	...
	surface.DrawRect(ScrW()/2-100, ScrH()-30, ScrH()/2 * charge, 15)
...
if CLIENT then
	...
	function ENT:Draw()
		...
		if self.aType && self.aType == "fire" && !self.dt.Collided then
			render.SetMaterial(self.Mat)
			render.DrawSprite(self:GetPos(), 32, 32, Color(255,0,0,255)) //Totally not stolen from Zero.
		end
		
		if IsValid(self.dt.Ent) && self.dt.Ent:IsPlayer() && self.dt.Ent:Alive() && self.dt.Grappled then	
...
if SERVER then
	...
	function ENT:Initialize()
		...
		if self.Arrowtype && self.Arrowtype == "explosive" then		
		...
		elseif self.Arrowtype == "fire" then
			self:SetColor(Color(255,0,0,255))
			self.Trail = util.SpriteTrail(self, 0, Color(255,0,0), false, 20, 1, 1.5, 0.01, "trails/lol.vmt")
		...
		elseif self.Arrowtype == "normal" then
			self.Trail = util.SpriteTrail(self, 0, Color(255,255,255), false, 5, 1, 0.3, 0.01, "trails/lol.vmt")
		end
	...
	function ENT:Think()
		...
		if self.Arrowtype == "grapple" && self.dt.Grappled && IsValid(self.Owner) then
			...
			if self.Owner:KeyDown(IN_RELOAD) || ( self.Owner:IsPlayer() && !self.Owner:Alive() && self.Hit ) then
	...
	function ENT:PhysicsCollide(data, physobj)
		...
		if tr.HitSky || !self:IsInWorld() then self:Remove() end
		
		if IsValid(self) && self.Arrowtype && !self.Hit then
			...
			if IsValid(ent) && !data.HitEntity:IsWorld() then
			
				if IsValid(self.Owner) then ent.FireCreditor = self.Owner end
				
				if !ent:IsPlayer() && !ent:IsNPC() && ent:GetClass() ~= "rj_arrow" then
			...
			if self.Arrowtype == "explosive" then
				...
				if IsValid(self.Inflictor) then
					util.BlastDamage(self.Inflictor, self.Owner, self:GetPos(), 100, 1000) 
					else
					util.BlastDamage(self, self.Owner, self:GetPos(), 100, 1000) 
				end
			...
			elseif self.Arrowtype == "fire" then
				...
				if IsValid(ent) then				
					if ((ent:IsNPC() || ent:IsPlayer()) && !self.PinData) then
						ent:Ignite(20,25)
			...
			if !self:GetParent():IsValid() && data.HitEntity:IsWorld() then
				self:SetPos(data.HitPos)
			end
			...
			if IsValid(ent) then
				...
				if ent:IsNPC() || ent:IsPlayer() then
					...
					if ent:Health() <= dmg:GetDamage() && trw.Hit then
	...
	local function FireKillCredit(ent,dmginfo)
		...
		if IsValid(attacker) && (attacker == inflictor) then
			if attacker:GetClass() == "entityflame" && IsValid(ent.FireCreditor) then
